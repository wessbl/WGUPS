...
class Package:
    def __init__(self, id, address, city, zip, deadline, mass, status):
        ...
        self.city = city
        self.zip = zip
        self.deadline = deadline    #TODO change format
        self.mass = mass
        self.status = status
...
class pkg_hash_table:
    def __init__(self, size):
        self.size = size
        self.arr = [pkg_node(Package(-1, "", "", "", "", "", ""))]  # -1 is the sign of an empty bucket)
        for i in range(size - 1):
            self.arr.append(pkg_node(Package(-1, "", "", "", "", "", "")))
...
class Map:
    ...
    def lookup(address, zip):
        for l in Map.locations:
            if l.address == address and l.zip == zip:
                return l.id
        return -1
